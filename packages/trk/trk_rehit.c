/*      subroutine trk_rehit */
/*
 *_begin_doc
 *  RCS ID string
 *  $Id: trk_rehit.c,v 1.7 1999/02/02 23:21:15 feuerbac Exp $
*/
/*
 *  Documentation for subroutine trk_rehit
 *
 *  Purpose:  
 *  --------
 ...
 *
 *_end_var
 *  executable code for routine trk_rehit:
 *  -------------------------------------
*/

#define MAX_WIRE 192

int trk_rehit_(vector3_t *ppos, int *psector, int *layer) {
  ...
  int wir_ind, max_cylw_ind;  /* location of a wire in the DCGW bank */
  ...
  if( (DCGM = (clasDCGM_t *)getGroup(&wcs_,"DCGM",sector)) && (DCGW) ) {
    ...
    p_sep = DCGM->dcgm[ilayer].p_sep*sqrt(1.-DCGM->dcgm[ilayer].z_norm*DCGM->dcgm[ilayer].z_norm); /* correct for rotations */
    max_cylw = DCGM->dcgm[ilayer].max_cylw;
    max_cylw_ind = ilayer*MAX_WIRE + max_cylw-1;
    ...
    theta_pl = atan2(DCGW->dcgw[max_cylw_ind].x_mid - x_curve, DCGW->dcgw[max_cylw_ind].z_mid - z_curve);
    max_cylw_z = DCGW->dcgw[max_cylw_ind].z_mid;
    /*    theta_pl = theta_start+(max_cylw-1)*d_theta; */
    /*    max_cylw_z = z_curve + sin(pi/2.0-theta_pl)*r_curve; */
    fkstereo = pi*stereo/180.;
    
    /*    if (ilayer < 12) {
     *      fprintf(stderr,"Evt: %d Sector: %d Layer %d: theta_pl: %f theta_c: %f\n",HEAD->head[0].nevent,sector,ilayer+1,theta_pl,theta_c);
     *    }
     */
    ...
    if ((max_wire != max_cylw)&&(theta_c > theta_pl)) {
      /*      fprintf(stderr,"max_cylw_z: %f,  from DCGW: %f",max_cylw_z,DCGW->dcgw[ilayer*MAX_WIRE +max_cylw-1].z_mid); */

      /*      fprintf(stderr,"\tp_sep: %f, corrected p_sep: %f\n",p_sep,p_sep*sqrt(1.-(DCGM->dcgm[ilayer].z_norm*DCGM->dcgm[ilayer].z_norm))); */

      wire = (((max_cylw_z-pos.z+0.5*p_sep)/p_sep)+max_cylw);
      /*      fprintf(stderr,"BEFORE sector: %d layer: %d wire: %d",sector, ilayer+1, wire); */
