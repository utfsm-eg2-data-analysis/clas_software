/* 
	mapf_diszero_float.c  : 30-Nov-1993 by R. Hackenburg,


	FORTRAN Callable, as:

	LOGICAL FUNCTION mapf_diszero_float( Filename, Subsystem, Itemname, ArrayLength, Array, Firsttime )
...
#ifndef vaxc
int mapf_diszero_float_(char *filename, char *subsystemname,
                   char *itemname, int *arraylength, float *array,
                   int *firsttime,
	           int filename_len, int subsystemname_len, int itemname_len)
#else
int mapf_diszero_float( char *filename, char *subsystemname,
                   char *itemname, int *arraylength, float *array,
                   int *firsttime,
	           int filename_len, int subsystemname_len, int itemname_len)
#endif
...
/*	Copy the FORTRAN-defined subsystemname into a local, C-defined one:  */
	...
	status = disIO_mapRequest( MAP_REQUEST_ZERO, filename_copy, subsystemname_copy, itemname_copy
