/* 
	mapf_put_char.c     : 28-Nov-1993 by R. Hackenburg,
...
#ifdef vaxc
int mapf_put_char( char *filename, char *subsystemname
                 , char *itemname, char *string
                 , int *firsttime
	         , int filename_len, int subsystemname_len, int itemname_len, int string_len )
#else
int mapf_put_char_( char *filename, char *subsystemname
                  , char *itemname, char *string
                  , int *firsttime
	          , int filename_len, int subsystemname_len, int itemname_len, int string_len )
#endif
/*
	...
	LOGICAL FUNCTION mapf_put_char( Filename, Subsystem, Itemname, String, Firsttime )

*  Inputs:
	CHARACTER*(*) Filename       !Map file name.
	CHARACTER*(*) Subsystemname  !Map subsytem name.
	CHARACTER*(*) Itemname       !Map item name.
	CHARACTER*(*) String         !String to be "put".
...
/*	Copy the FORTRAN-defined subsystemname into a local, C-defined one:  */
	...
	status = map_put_char( filename_copy, subsystemname_copy, itemname_copy, string_len, string, *firsttime );
