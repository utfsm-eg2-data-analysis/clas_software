/* 
	mapf_add_item.c    : 28-Nov-1993 by R. Hackenburg,
...
#ifdef vaxc
int mapf_add_item( char *filename, char *subsystemname,
                    char *itemname, int *arraylength, int *arraytype ,
	            int filename_len, int subsystemname_len, int itemname_len)
#else
int mapf_add_item_(char *filename, char *subsystemname,
                    char *itemname, int *arraylength, int *arraytype ,
	            int filename_len, int subsystemname_len, int itemname_len)
#endif
/*
	...
	LOGICAL FUNCTION mapf_add_item( Filename, Subsystem, Itemname, ArrayLength, ArrayType )
...
*  Inputs:
	...
	INTEGER       ArrayType      !Type of Array (0 for int, 1 for real, 2 for string).
...
/*	Copy the FORTRAN-defined subsystemname into a local, C-defined one:  */
	...
	status = map_add_item(filename_copy,subsystemname_copy,itemname_copy,*arraylength,
	                      *arraytype);
