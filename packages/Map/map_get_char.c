...
/*   --------------                                                    */
/*                                                                     */
/*         Created     : 18-May-1993    Author : Thom Sulanke          */
/*         Purpose     : get array of char values from the map.  if no */
     ...
     
{
  ...
  *vtime = -2;
  ...
  if ( strlen(subsystemname) > MAP_NAMELENGTH-1 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_char: Subsystem name (%s) longer than %d characters.",
		subsystemname,
		MAP_NAMELENGTH-1);
  ...
  if ( strlen(itemname) > MAP_NAMELENGTH-1 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_char: Item name (%s) longer than %d characters.",
		itemname,
		MAP_NAMELENGTH-1);
      return MAP_USER_ERROR_ARGUMENT;
    }
 ...
 tryagain:
  if ( ctry == 1 )
  ...
  else
    {
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_char: Retrying for time %d, subsystem (%s), item (%s).\n",
	       atime,
	       subsystemname,
	       itemname);
  ...
  if ( fd < 0 )
    {
      map_syserror(fd,
		   "map_get_char: can't open TheMap (%s).",
		   filename);
      return fd;
    }
  ...
  if ( length <= 0 || length > item.length )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_char: Array length (%d) incorrect for subsystem (%s),\n item (%s).  Should be not greater than %d.\n",
		length, 
		subsystemname,
		itemname,
		item.length);
  ...
  if ( item.type != 2 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_char: Array type (%d) for subsystem (%s), item (%s) \n requires call to different map_get_* routine.",
		item.type,
		subsystemname,
		itemname);
      map_close_map(fd);
  ...
  if ( (item.list == NULL_LOC) || (item.list < 0 && item.table_used == 0 ))
    {
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_char: No values for subsystem (%s), item (%s).\n",
	       subsystemname,
	       itemname);
      *vtime = -1;
      map_close_map(fd);
  ...
  if ( succ_loc == NULL_LOC )
      ...
      if ( ctry == 1 )
	{
      ...
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_char: No values at time %d for subsystem (%s), item (%s).\n",
	       atime,
	       subsystemname, 
	       itemname);
      *vtime = -1;
  ...
  *vtime = succ_time;
  ...
  status = map_read(&header,sizeof header,fd,succ_loc,swaparrayheader);
  if ( status != MAP_OK ) 
    {
      map_close_map(fd);
      return status;
  ...
  *olength = length;
  if ( *olength > header.length ) *olength = header.length;
  status = map_read(array,(*olength)*sizeof(char),fd,succ_loc+ARRAY_VALUES_OFFSET,swapchararray);
  if ( status != MAP_OK ) 
    {
      map_close_map(fd);
      return status;
