/* 
	mapf_disadd_float.c  : 30-Nov-1993 by R. Hackenburg,

	FORTRAN Callable, as:

	LOGICAL FUNCTION mapf_disadd_float( Filename, Subsystem, Itemname, ArrayLength, Array, Firsttime )
...
*  Inputs:
	...
	REAL          Array(*)       !Array of item-values.
...
#ifndef vaxc
int mapf_disadd_float_(char *filename, char *subsystemname,
                   char *itemname, int *arraylength, int *array, 
                   int *firsttime ,
	           int filename_len, int subsystemname_len, float itemname_len)
#else
int mapf_disadd_float( char *filename, char *subsystemname,
...
/*	Copy the FORTRAN-defined subsystemname into a local, C-defined one:  */
	...
	status = disIO_mapRequest( MAP_REQUEST_ADD, filename_copy, subsystemname_copy, itemname_copy
	                         , *firsttime, MAP_DATATYPE_FLOAT, *arraylength, array );
