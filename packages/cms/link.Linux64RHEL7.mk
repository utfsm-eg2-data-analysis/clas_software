...
RECSIS_SHARED_LIBS  += -lstdc++ -lnsl -ldl -lgfortran -ltcl -lmysqlclient
...
ifdef USEBOSIO
  GSIM_INT_LIBS0 += bosio
else
  GSIM_INT_LIBS0 += fputil bos fpack
endif
...
ifndef MAP
  GSIM_INT_SHARED_LIBS += -ltcl -lmysqlclient
  GSIM_BAT_SHARED_LIBS += -ltcl -lmysqlclient
endif
...
ifdef NO_MOTIF
  ifeq ($(ARCH),32bit)
    GSIM_INT_SHARED_LIBS += -L$(SOFT)/lib -lXp -lXext -lX11 
  endif
  ifeq ($(ARCH),64bit)
    GSIM_INT_SHARED_LIBS += -L/usr/lib64 -lXp -lXext -lX11 
  endif
else
  ifeq ($(ARCH),32bit)
    GSIM_INT_SHARED_LIBS += -L$(SOFT)/lib -lXm -lXt -lSM -lICE -lX11 -lXext -lXaw -lXp 
  endif
  ifeq ($(ARCH),64bit)
    GSIM_INT_SHARED_LIBS += -L/usr/lib64 -lXm -lXt -lSM -lICE -lX11 -lXext -lXaw -lXp 
  endif
endif
...
ifeq ($(ARCH), 32bit)
  GSIM_BAT_SHARED_LIBS += -L$(SOFT)/lib -lX11 -lblas
  SWISH_SHARED_LIBS    += /usr/lib/libdl.so
  CED_SHARED_LIBS      += -L$(SOFT)/lib -lXext -lXaw -lXp
  PB_INT_SHARED_LIBS   += -L$(SOFT)/lib -lXm -lX11 -lXt
  PB_BAT_SHARED_LIBS   += -L$(SOFT)/lib -lX11 -lXt
endif
ifeq ($(ARCH), 64bit)
  GSIM_BAT_SHARED_LIBS += -L/usr/lib64 -lX11 -lblas
  SWISH_SHARED_LIBS    += /usr/lib64/libdl.so
  CED_SHARED_LIBS      += -L/usr/lib64 -lXext -lXaw -lXp
  PB_INT_SHARED_LIBS   += -L/usr/lib64 -lXm -lX11 -lXt
  PB_BAT_SHARED_LIBS   += -L/usr/lib64 -lX11 -lXt
endif
