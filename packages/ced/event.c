...
static Hv_Widget        evhead, evtage, evtagt, evtgtl, evtgtr, evmcin, evst, evstn0, evstn1, evmcvx, evmctk, evecpi;  
...
char   *bosBankNameTGTL = "TGTL";
char   *bosBankNameTGTR = "TGTR";
...
/* now SC */
	...
	accsc[sect].scale = 1;
	for (scint = 0; scint < num_scint_used; scint++)
...
/* once only banks */
	...
	if (old_startcntr) {
	  ...
	  evtgtl = Hv_StandardButton(rc, "TGTL", BankCB);
	  evtgtr = Hv_StandardButton(rc, "TGTR", BankCB);
...
/* tagger E  data ? */
    ...
    j = 0;
    bosIndexTGTR = mlink_(bcs_.iw,bosBankNameTGTR, &j, 4);
    if (bosIndexTGTR != 0) {
	banksize = bosBankSize(bosIndexTGTR);
	bosNumTGTR = banksize/sizeof(TGTRData);
	bosTGTR = (TGTRDataPtr)(&(bcs_.iw[bosIndexTGTR]));
    }
    else {
	j = 1;
	bosIndexTGTR = mlink_(bcs_.iw,bosBankNameTGTR, &j, 4);
    ...
    j = 0;
    bosIndexTGTL = mlink_(bcs_.iw,bosBankNameTGTL, &j, 4);
    if (bosIndexTGTL != 0) {
	banksize = bosBankSize(bosIndexTGTL);
	bosNumTGTL = banksize/sizeof(TGTLData);
	bosTGTL = (TGTLDataPtr)(&(bcs_.iw[bosIndexTGTL]));
    }
    else {
	j = 1;
	bosIndexTGTL = mlink_(bcs_.iw,bosBankNameTGTL, &j, 4);
    ...
    
{
    ...
    esize += bosBankSize(bosIndexTGTL);
    esize += bosBankSize(bosIndexTGTR);
    ...
    
{
	...
	DumpTGTLBank(bosTGTL, bosNumTGTL);
    }
    
    else if (w == evtgtl) {
	DumpTGTRBank(bosTGTR, bosNumTGTR);
    }
...
/*	Hv_SetSensitivity(evconvert,  fileopen); */
    ...
    if (old_startcntr) {
      ...
      Hv_SetSensitivity(evtgtr, (bosIndexTGTR > 0));
      Hv_SetSensitivity(evtgtl, (bosIndexTGTL > 0));
