...

{
    ...
    if (geom_runnumber <= 1 && runnumber != 1) {
      old_startcntr = (runnumber <= LAST_RUN_OLD_ST)? True : False;

      /* initialized Scintillators if runnumber in different range than during first initialization */

      if (runnumber <= LAST_RUN_COUPLED_SC && num_scint_used != NUM_SCINT_OLD ) {
	
	num_scint_used = NUM_SCINT_OLD;
	...
	for (sect = 0; sect < 6; sect++)
      ...
      if (runnumber > LAST_RUN_COUPLED_SC && num_scint_used != NUM_SCINT) {
	
	num_scint_used = NUM_SCINT;

	fp = GetGeometryFile();

	for (sect = 0; sect < 6; sect++) {
	  if (Hv_ReportDatabaseSearch(fp, sc57recnames[sect], True))
	    ReadScintGeometry(fp, sect);
	  else {
	    if (Hv_ReportDatabaseSearch(fp, screcnames[sect], True)) {
	      fprintf(stderr,"geometry file only contains SC geometry before decoupling backward counters\n");
	      num_scint_used = NUM_SCINT_OLD;
	      ReadScintGeometry(fp, sect);
	    }
	  }
...
/* got the file, now try to read it, starting with version */
...
/*Scintillator Data*/
  ...
  if (geom_runnumber > 0) {
    if (geom_runnumber <= LAST_RUN_OLD_ST)     old_startcntr = True;
    if (geom_runnumber <= LAST_RUN_COUPLED_SC) num_scint_used = NUM_SCINT_OLD;
  }

  for (sect = 0; sect < 6; sect++) {

    if (num_scint_used == NUM_SCINT) {
      if (Hv_ReportDatabaseSearch(fp, sc57recnames[sect], True))
	ReadScintGeometry(fp, sect);
      else {
	if (Hv_ReportDatabaseSearch(fp, screcnames[sect], True)) {
...
/*keep in mind that "C" forces us to start indices at zero*/
  ...
  printf("read CC geometry sect %d\n",sect+1);
...
/*keep in mind that "C" forces us to start indices at zero*/
  ...
  printf("read LAC geometry sect %d\n",sect+1);
...
/*keep in mind that "C" forces us to start indices at zero*/
  ...
  printf("read EC geometry sect %d\n",sect+1);
...
/*keep in mind that "C" forces us to start indices at zero*/
  ...
  printf("read DC geometry sect %d\n",sector+1);
...
/*keep in mind that "C" forces us to start indices at zero*/
  ...
  printf("read SC geometry sect %d for %d counters\n",sect+1,num_scint_used);
  
  for (i=0;  i < num_scint_used;  i++) {
      Hv_GetNextNonComment(fp, line);
