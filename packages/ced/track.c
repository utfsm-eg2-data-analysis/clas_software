...
int  GetNumHBTracks() {
    ...
    if (numhbla != numhdpl) {
      if (numhbla>0 && numhdpl>0) fprintf(stderr, "WARNING: number of tracks in HBLA and HDPL differ. Using max.\n");
...
int  GetNumTBTracks() {
    ...
    if (numtbla != numtdpl) {
      if(numtbla>0 && numtdpl>0) fprintf(stderr, "WARNING: number of tracks in TBLA and TDPL differ. Using max.\n");
...
/*
 * loop through all the HDPLs and return sect for the 
 * one with matching ID, else return -1
 */
    ...
    fprintf(stderr, "WARNING: did NOT find HBLA nor HDPL for trackid: %d\n", trackid); 
...
/*
 * loop through all the TDPLs and return sect for the 
 * one with matching ID, else return -1
 */
    ...
    fprintf(stderr, "WARNING: did NOT find TBLA nor TDPL for trackid: %d\n", trackid);
