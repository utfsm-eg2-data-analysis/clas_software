...
/* see if there is a t counter hit that
 ...
 its range */
    ...
    if (tlhits != NULL && trhits != NULL && numl > 0 && numr > 0) {
        for (j = 0; j < numl; j++) {
	  BreakShort(tlhits->id, &id, &dummy);

	  if ((id > 1) && (id <= Tagger.numTC)) {
	    id--;

	    if ((tlhits->tdc > tagttdcmin) && (tlhits->tdc < tagttdcmax) &&
		(eid >= Tagger.e_lo1[id]) && (eid <= Tagger.e_hi1[id])) { 
	      lfound = True;
	    }
	  }
	  tlhits++;
	...
	if (lfound) {
	  for (j = 0; j < numr; j++) {
	    BreakShort(trhits->id, &id, &dummy);

	    if ((id > 1) && (id <= Tagger.numTC)) {
	      id--;

	      if ((trhits->tdc > tagttdcmin) && (trhits->tdc < tagttdcmax) &&
		  (eid >= Tagger.e_lo1[id]) && (eid <= Tagger.e_hi1[id])) { 
		return True;
	      }
	    }
...
/* first the E counters */
	...
	for (j = 0; j < num; j++) {
		...
		(ehits->tdc > tagetdcmin) && (ehits->tdc < tagetdcmax)) {
...
/* now the T counters: first try TAGT hits then TGTL, TGTR */
    ...
    if ((tlhits != NULL) && (num > 0)) {
	GetTaggerMainTRect(View, &rect);

	for (j = 0; j < num; j++) {
	    BreakShort(tlhits->id, &id, &dummy);
	    if ((id > 0) && (id <= Tagger.numTC) &&
		(tlhits->tdc > tagttdcmin) && (tlhits->tdc < tagttdcmax)) {
	      GetTaggerTRect(View, id, &rect);
	      yc = (rect.top + rect.bottom)/2;
	      rect.bottom = yc-2;
	      Hv_FixRectWH(&rect);
    ...
    if ((trhits != NULL) && (num > 0)) {
	GetTaggerMainTRect(View, &rect);

	for (j = 0; j < num; j++) {
	    BreakShort(trhits->id, &id, &dummy);
	    if ((id > 0) && (id <= Tagger.numTC) && 
		(trhits->tdc > tagttdcmin) && (trhits->tdc < tagttdcmax)) {
	      GetTaggerTRect(View, id, &rect);
	      yc = (rect.top + rect.bottom)/2;
	      rect.top = yc+2;
	      Hv_FixRectWH(&rect);
...
/* a tagger is malloced for each sector view. 
