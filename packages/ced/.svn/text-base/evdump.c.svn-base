...
void DumpSTBank(STDataPtr  hits,
		       int        num)
    
{
    int            j;
    char          *header = "\t sector\t  tdc\t   adc\n\t-----------------------------\n";
    char           line[EV_LINE_SIZE];
    
    if ((hits == NULL) || (num == 0))
	return;
    
    sprintf(line, "Start Counter Number of Hits = %d\n", num);
    Hv_AddLineToScrolledText(evtext, &evpos, line);
    Hv_AddLineToScrolledText(evtext, &evpos, header);
    ...
    
{
    ...
    sprintf(line, "Number of Tagger E Counter Hits = %d\n", num);
    Hv_AddLineToScrolledText(evtext, &evpos, line);
    Hv_AddLineToScrolledText(evtext, &evpos, header);
...
void DumpTGTLBank(TGTLDataPtr  hits,
                  int          num) {

  int            j;
  char          *header = "\t T counter\t  tdc\n\t-----------------------------\n";  char           line[EV_LINE_SIZE];

  if ((hits == NULL) || (num == 0)) {
    return;
  }

  sprintf(line, "Number of Tagger T counter hits (left) = %d\n", num);
  Hv_AddLineToScrolledText(evtext, &evpos, line);
  Hv_AddLineToScrolledText(evtext, &evpos, header);
...
void DumpTGTRBank(TGTRDataPtr  hits,
                  int          num) {
  ...
  sprintf(line, "Number of Tagger T counter hits (right) = %d\n", num);
  Hv_AddLineToScrolledText(evtext, &evpos, line);
  Hv_AddLineToScrolledText(evtext, &evpos, header);
...
void DumpECBank(ECDataPtr  hits,
		       int     num)
