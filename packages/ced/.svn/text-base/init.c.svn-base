...
int            geom_runnumber = 0;   /* run number for geometry init:
					default=take runnumber */
...
Boolean        old_startcntr = False;  /* default: new start counter */

int            num_scint_used = NUM_SCINT; /* default: decoupled paddles */
...
/*     /\* fprintf(stderr, "Maps directory: %s\n", dir); *\/ */
     fprintf(stdout, "DumpMap() function requires stuff not implemented in CalDBMap interface.\n"); 
/*     /\* dc geometry *\/ */
...
/* from et flag? */
	...
	else if (strcmp(Hv_argv[i], "-r") == 0) {
	  if ((i < (Hv_argc-1)) && !RecognizedFlag(Hv_argv[i+1])) {
	    geom_runnumber = atoi(Hv_argv[i+1]);
	    i++;
	  }
	}
...

{
    ...
    fprintf(stderr, "\n\t-r  <runnumber>\n");
    fprintf(stderr, "\t\tSets geometry for run <runnumber>.\n");
