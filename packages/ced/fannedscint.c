...
void DrawStartHits(Hv_View View,
	...
	st_id = (short)(hitsN0->id-1);
	sect = st_id/4;
	StartCounterSubPoly(View, sect, poly1, poly2);
	splitStartCounterPoly1(poly1, subpoly1);

	if (hitsN0->tdc > 0) {
	  Hv_FillPolygon(subpoly1[st_id % 4], 3, True, Hv_red, Hv_black);
	}
	hitsN0++;
      }

      for (i = 0; i < bosNumSTN1; i++) {
	st_id = (short)(hitsN1->id-1);
	sect = st_id/4;
	StartCounterSubPoly(View, sect, poly1, poly2);
	splitStartCounterPoly2(poly2, subpoly2);
	
	if (hitsN1->adc > 0) {
	  Hv_FillPolygon(subpoly2[st_id % 4], 4, True, Hv_red, Hv_black);
	}
	hitsN1++;
      }
    }
