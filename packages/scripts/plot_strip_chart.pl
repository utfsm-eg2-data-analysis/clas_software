...
@run_list = ();
@var_list = ();
$dbdir = "/work/clas/production/g1/pass1/database";
$beam = 0;
$torus = 0;
$run_min = 1;
$run_max = 999999;
...
foreach (@ARGV) {
	...
	$norm_spec=$_;
	$norm_spec =~ s/^-N//;
	print "$0: normalizing using: $norm_spec";
    } elsif (/^-A/) {
	$avg = 1;
    } elsif (/^-S/) {
	$same = 0;
...
#	chop($run);
	$run =~ s/\.db$//;
	if ($beam || $torus) {
	    dbmopen(%RUN, "$run", 0444) || warn "$0: Couldn't open $run\n";
...
if(1==$norm_special){
    $title="normalized using $norm_spec";
    print OUT "# $title\n";
}
if (1==$norm_farad) {
    $title="normalized using faraday cup";
    print OUT "# $title\n";
}
...
# Initialize hash
	...
	if ($norm_farad && !$first) {
	  ...
	  if ($gate_farad_coul == 0) {
        ...
        } elsif ($norm_special && !$first) {
	  if  ($RUN{$norm_spec} == 0) {
	    next;
	  } else {
	    $entry{"NES"} += $RUN{$norm_spec};
	  }
        }
	$entry{$var} += $temp;
	$first = 1;
...
print GNUOUT "set term postscript color\n";
if ($ymin < $ymax){
...
if (1==$norm_special || 1==$norm_farad){
    print GNUOUT "set title \"$title\"\n";
    print GNUOUT "show title\n";
}
...
sub PrintUsage {
    ...
    print "\t-nr \t\tNormalize results to raw # of events.\n";
    print "\t-N<norm> \t\tNormalize results to <norm>.\n";
