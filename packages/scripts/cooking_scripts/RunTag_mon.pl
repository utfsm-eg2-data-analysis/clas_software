...
#         $db_mysql      = Flag to use the MYSQL db instead of the perl db.
  ...
  if ($dispatcher){
    $photon_cmd = "$clas_bin/tag_mon -R -D -P -i ";
  } else{
    $photon_cmd = "$clas_bin/tag_mon -R -P -i -t$tripdir/clas_0$basename.trip ";
  }
...
# MYSQL db info is stored using the -z flag
  ...
  $photon_cmd .= " -otagm$basename.hbook $files";
  ...
  if (!$db_mysql) {
    ...
    $RUN{"tag_mon_cmd"} = $photon_cmd;

#    if (-r "tagm$basename.txt") { 
#	open(PHM_TXT, "tagm$basename.txt") || print  "$0: Could not open tagm$basename.txt\n";
#    }
...
#    MoveFile("tagm*txt", $OutPath, "tag_mon text file", $db_file);
  MoveFile("tagm*hbook", $OutPath, "tag_mon histo file", $db_file);
