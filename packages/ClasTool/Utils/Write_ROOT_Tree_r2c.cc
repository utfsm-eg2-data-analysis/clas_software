...
//          Mod : Orlando Soto USM 10/20/2015
//
//
/////////////////////////////////////////////////////////////////////
// Begin_Html <!--
...
{
  TROOT write_tree("WriteRootDst","Program for creating the root DST from root NT10 files.");

  Int_t comp=__COMPRESS__;
  int   iostatus;
  int   iCount=0;
  int   filenum;
  char  rootInputFileName[256]; //MOD
  TFile *rootInputFile; //MOD
  TFile *hFile;
  TTree *tree;
  ...
  ///////////////////////////////////////////////////////////////////
  // Initialize ??? file name, open ??? File and Read all events.
  ///////////////////////////////////////////////////////////////////
  ...
  for(filenum=1; filenum<argc; filenum++)
  {
    ...
    strcpy(rootInputFileName, argv[filenum]); 
    if(gSystem->AccessPathName(rootInputFileName, kReadPermission))
    {
      cerr << "*** ERROR: The file " << rootInputFileName << " can not be read." << endl;
      continue;
    }

    rootInputFile = new TFile(rootInputFileName); //MOD
    if(!rootInputFile)
    {
      cout << "File " << rootInputFileName << " could not be opened. ";
      continue;
    }

    //MOD////// Obtaining the tree in the input file
    TTree *data_tree;
    TIter nextkey(rootInputFile ->GetListOfKeys());
    TKey *key, *oldkey=0;
    ...
    if(data_tree==NULL) exit(1);
    tree_str *dataIn_tree = new tree_str(data_tree); // Initialize Branch of the tree to be read.

    Long64_t entry_count=0;
    while((dataIn_tree->GetEntry(entry_count) != 0) && (iCount<nEventsWrite))
    {
      entry_count++;
      Clear_Clones();
      ...
      // EventType=0;
      // HEAD kfileHEADER(0);
      if(dataIn_tree->npart > 0)
      {
        if((dataIn_tree->evntid==0)
          || (dataIn_tree->evntid==1)
          || (dataIn_tree->evntid==10))
        {
        ...
        {
          if((dataIn_tree->evntype==1)
            || (dataIn_tree->evntype==2)
            || (dataIn_tree->evntype==4))
          {
            EventType=PHYSICEVNT;
          }
          else if(dataIn_tree->evntype==10)
          {
      ...
      if((EventType==PHYSICEVNT))
      {
        ...
        if(dataIn_tree->gpart > 0)
        {
          for(int j=0; j<dataIn_tree->gpart; j++)
          {
            Fill_EVNT_Bank(EVNT_p, dataIn_tree, j);
            if(debug_flag > 1)
            {
              cout << "\nEVNT Row: " << j << endl;
              EVNT_p->Print();
        ...
        if(DO_GSIM_flag)
        {
          ...
          if(dataIn_tree->mcnentr > 0)
          {
            for(int j=0; j<dataIn_tree->mcnentr; j++)
            {
              Fill_GSIM_Bank(GSIM_p, dataIn_tree, j);
              if(debug_flag > 1)
              {
                cout << "\nGSIM Row: " << j << endl;
                GSIM_p->Print();
        ...
        if(dataIn_tree->ec_part > 0)
        {
          for(int j=0; j<dataIn_tree->ec_part; j++)
          {
            Fill_ECPB_Bank(ECPB_p, dataIn_tree, j);
        ...
        if(dataIn_tree->sc_part > 0)
        {
          for(int j=0; j<dataIn_tree->sc_part; j++)
          {
            Fill_SCPB_Bank(SCPB_p, dataIn_tree, j);
        ...
        if(dataIn_tree->dc_part > 0)
        {
          for(int j=0; j<dataIn_tree->dc_part; j++)
          {
            Fill_DCPB_Bank(DCPB_p, dataIn_tree, j);
        ...
        if(dataIn_tree->cc_part > 0)
        {
          for(int j=0; j<dataIn_tree->cc_part; j++)
          {
            Fill_CCPB_Bank(CCPB_p, dataIn_tree, j);
        ...
        if(dataIn_tree->lac_part > 0)
        {
          for(int j=0; j<dataIn_tree->lac_part; j++)
          {
            Fill_LCPB_Bank(LCPB_p, dataIn_tree, j);
    ...
    if(debug_flag) cout << "Close Root File \n";
    rootInputFile->Close();
    if(debug_flag) cout << "Close Root OK\n";
  }//for(filenum=1; filenum < argc; filenum++)
...
void Print_Usage()
{
  cout << " This Program converts a ROOT (NT10)-format file to a ROOT (ClasTool)-format\n";
  cout << " Usage: WriteRootDst_R2C [-o outputfile] [-n events] infile1 infile2 ...\n";
