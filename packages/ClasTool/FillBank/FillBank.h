////////////////////////////////////////////////
//    Fill from BOS function prototypes ...
/////////////////////////////////////////////////
#include "tree_str.h"
void  Fill_HEADER_Bank(THEADERClass *gcHEADER);
// TARGET_INCLUDE2
void  Fill_FBPM_Bank(TFBPMClass *gcFBPM,FBPM *pFBPM , int nrow);

void  Fill_ICPB_Bank(TICPBClass *gcICPB,tree_str* pICPB , int nrow);
void  Fill_ICPB_Bank(TICPBClass *gcICPB,ICPB *pICPB , int nrow);

void  Fill_ICHB_Bank(TICHBClass *gcICHB,ICHB *pICHB , int nrow);

void  Fill_EVNT_Bank(TEVNTClass *gcEVNT, tree_str* pEVNT, int nrow);
void  Fill_EVNT_Bank(TEVNTClass *gcEVNT, EVNT *pEVNT, int nrow);

void  Fill_ECPB_Bank(TECPBClass *gcECPB, tree_str* pECPB, int nrow);
void  Fill_ECPB_Bank(TECPBClass *gcECPB, ECPB *pECPB, int nrow);

void  Fill_SCPB_Bank(TSCPBClass *gcSCPB, tree_str* pSCPB, int nrow);
void  Fill_SCPB_Bank(TSCPBClass *gcSCPB, SCPB *pSCPB, int nrow);

void  Fill_DCPB_Bank(TDCPBClass *gcDCPB, tree_str* pDCPB, int nrow);
void  Fill_DCPB_Bank(TDCPBClass *gcDCPB, DCPB *pDCPB, int nrow);

void  Fill_CCPB_Bank(TCCPBClass *gcCCPB, tree_str* pCCPB, int nrow);
void  Fill_CCPB_Bank(TCCPBClass *gcCCPB, CCPB *pCCPB, int nrow);

void  Fill_LCPB_Bank(TLCPBClass *gcLCPB, tree_str* pLCPB, int nrow);
void  Fill_LCPB_Bank(TLCPBClass *gcLCPB, LCPB *pLCPB, int nrow);

void  Fill_HEAD_Bank(THEADClass *gcHEAD, HEAD *pHEAD, int nrow); //HEADER
void  Fill_HEAD_Bank(THEADClass *gcHEAD, tree_str* pHEAD, int nrow); //HEADER

void  Fill_HLS_Bank(THLSClass *gcHLS, HLS *pHLS, int nrow);

void  Fill_S1ST_Bank(TS1STClass *gcS1ST, S1ST *pS1ST, int nrow);

void  Fill_TRGS_Bank(TTRGSClass *gcTRGS, TRGS *pTRGS, int nrow);

void  Fill_GSIM_Bank(TGSIMClass *gcGSIM,tree_str* pPART , int nrow);
void  Fill_GSIM_Bank(TGSIMClass *gcGSIM,PART *pPART , int nrow);
void  Fill_GSIM_Bank(TGSIMClass *gcGSIM,MCTK *pMCTK, MCVX *pMCVX , int nrow);

void  Fill_TGBI_Bank(TTGBIClass *gcTGBI,TGBI *pTGBI , int nrow);

void  Fill_STPB_Bank(TSTPBClass *gcSTPB, tree_str* pSTPB, int nrow);
void  Fill_STPB_Bank(TSTPBClass *gcSTPB, STPB *pSTPB, int nrow);

void  Fill_TGPB_Bank(TTGPBClass *gcTGPB, TGPB *pTGPB, int nrow);

void  Fill_TAGR_Bank(TTAGRClass *gcTAGR, TAGR *pTAGR, int nrow);

void  Fill_VERT_Bank(TVERTClass *gcVERT, VERT *pVERT, int nrow);

void  Fill_MVRT_Bank(TMVRTClass *gcMVRT, MVRT *pMVRT, int nrow);

void  Fill_TBER_Bank(TTBERClass *gcTBER, TBER *pTBER, int nrow);

void  Fill_MCHD_Bank(TMCHDClass *gcMCHD, MCHD *pMCHD, int nrow);
